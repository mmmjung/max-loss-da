...
def make_vocab(dataset, tokens_to_add, max_vocab_size=50000, min_freq=1):
    ...
    if tokens_to_add:
        for token in tokens_to_add:
            ...
            if lower_token not in uniq_tokens:
                vocab[lower_token] = len(vocab)
...
def convert_seq(batch, device=None, with_label=True):
    ...
    if with_label:
        return [to_device_batch([x for x, _ in batch]),
