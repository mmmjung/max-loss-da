...
def get_other_text_dataset(name, train_path, test_path, tokens_to_add=None, vocab=None, shrink=1,
                           char_based=False, seed=777):
    assert(name in ['TREC', 'stsa.binary', 'stsa.fine',
                    'custrev', 'mpqa', 'rt-polarity', 'subj'])

    train = read_other_dataset(
        train_path, shrink=shrink, char_based=char_based)

    if test_path:
        test = read_other_dataset(
            test_path, shrink=shrink, char_based=char_based)
    else:
    ...
    if vocab is None:
        print('constract vocabulary based on frequency')
        vocab = make_vocab(train, tokens_to_add)
